...
local function downloadDlc( rootUrl, listener)
	...
	local function manifestDownload(event)
		...
		elseif event.phase == "ended" then
			...
			mf:close( )
			-- and read values from parsed json: total size and which files to download
	...
	-- donwload 'manifest.json' file, containing total size and which files to download
	-- this is, basically, description of what has to be donwloaded
